...
class PDFExtractor(IExtractor):
    ...
    # =================================================================================
    # == NEW 1B METHOD: EFFICIENT FULL TEXT EXTRACTION FOR SEMANTIC ANALYSIS
    # =================================================================================
    def extract_full_text(self, pdf_path: str) -> str:
        """
        Extracts the full, plain text content from the entire PDF document.
        This method is optimized for speed and is intended for use in semantic
        analysis pipelines where only the text content is required.
        ...
        try:
            with fitz.open(pdf_path) as doc:
                for page in doc:
                    # page.get_text("text") is the fastest method for raw text extraction
                    full_text += page.get_text("text") + "\n"
            logging.info(f"Extracted full text ({len(full_text)} chars) from '{pdf_path}' for 1B analysis.")
            return full_text
        except Exception as e:
            logging.error(f"Failed to extract full text from {pdf_path}: {e}", exc_info=True)
            return ""
